...
int run_bone(struct mars_peerinfo *peer, struct xio_dent *remote_dent)
{
	...
	if (mars_stat(marker_path, &local_stat, true) >= 0) {
		XIO_DBG("marker '%s' exists, ignoring '%s'\n", marker_path, remote_dent->d_path);
		goto done;
	}
	...
	if (stat_ok) {
		...
		if ((remote_dent->stat_val.mode & S_IRWXU) !=
		   ...
		   update_ctime) {
			...
			XIO_DBG("chmod '%s' 0x%xd -> 0x%xd\n", remote_dent->d_path, newmode & S_IRWXU, remote_dent->stat_val.mode & S_IRWXU);
			newmode &= ~S_IRWXU;
		...
		if (remote_dent->stat_val.uid != local_stat.uid && update_ctime) {
			XIO_DBG("lchown '%s' %d -> %d\n", remote_dent->d_path, local_stat.uid, remote_dent->stat_val.uid);
	...
	if (S_ISDIR(remote_dent->stat_val.mode)) {
		...
		if (!stat_ok || update_mtime) {
			...
			XIO_DBG("create symlink '%s' -> '%s' status = %d\n", remote_dent->d_path, remote_dent->link_val, status);
			run_trigger = true;
		}
...
static
bool _next_is_acceptable(struct mars_rotate *rot, struct xio_dent *old_dent, struct xio_dent *new_dent)
{
...
static
int make_res(void *buf, struct xio_dent *dent)
{
...
static
int kill_res(void *buf, struct xio_dent *dent)
{
	struct mars_rotate *rot = dent->d_private;
